...
//
//	/**
//	 * 将int数值转换为byte数组，本方法适用于(低位在后，高位在前)的顺序
//	 * 
//	 * @param value
//	 *            要转换的int值
//	 * @return byte数组
...
//	}
	...
	/**
	 * 将long数值转换为占四个字节的byte数组，本方法适用于(低位在前，高位在后)的顺序。
	 * 
	 * @param value
	 *            要转换的long值
	 * @return byte数组
	...
	/**
	 * 将long数值转换为byte数组，本方法适用于(低位在后，高位在前)的顺序
	 * 
	 * @param value
	 *            要转换的long值
	 * @return byte数组
	 */
	public static byte[] longToBytes2(long value, int length) {
		byte[] src = new byte[length];
		for (int i = 0; i < length; i++) {
			src[i] = (byte) ((value >> (8 * (length - i - 1))) & 0xFF);
		}
	...
	/**
	 * byte数组中取long数值，本方法适用于(低位在前，高位在后)的顺序，和和bytesToLong（）配套使用
	 * 
	 * @param src
	 *            byte数组
	 * @param offset
	 ...
	 * @return int数值
	 */
	public static long bytesToLong(byte[] src, int offset, int length) {
...
//		}
	 ...
	 */
	public static long bytesToInt2(byte[] src, int offset, int length) {
	 ...
	 */
	public static long bytesToLong2(byte[] src, int offset, int length) {
		long value = ((src[offset] & 0xFF) << 8 * length);
		for (int i = 1; i < length; i++) {
			value = (int) (value | (src[offset + i] & 0xFF) << 8 * (length - i));
		}
		return value;
